...
def find_factor_base(n, smoothness_bound = 0, SURPRESS_WARNING = False):
    ...
    for p in sympy.primerange(3, smoothness_bound):
        ...
        if legendre_symbol == 0 and not SURPRESS_WARNING:
            print("A prime below the smoothness bound divides n")
    ...
    while len(smooth_f_r_i) < len(factor_base) + OVERKILL and r_i <= math.floor(n**0.5)+1 + SIEVE_INTERVAL_SIZE:
        ...
        if abs(LOG_FUNCTION(f_r_i) - sum_of_prime_logs) < LOG_TOLERANCE:
            ...
            if np.prod(list(map(pow, factor_base, f_r_i_factor_powers)), dtype='int64') == f_r_i:
                ...
                if PRINT_INFO:
                    print("f(r_{}) = {} is smooth".format(r_i-math.floor(n**0.5), f_r_i))
...
def gaussian_elimination_step(n, smooth_f_r_i, factor_base, PRINT_MATRICES = False, PRINT_INFO = True,
    ...
    marks = np.zeros((no_of_smooth_f_r_i, no_of_smooth_f_r_i)) #[column affected by row operations, column with pivot]
            ...
            if exp_matrix_b2[j,i] == 1:
                for k in range(no_of_smooth_f_r_i):   #For each column other than the current one
                    ...
                    if exp_matrix_b2[j,k] == 1:   #If the column addition is needed
                        marks[k,:] = marks[i,:]
                        marks[k,i] = 1
                        marks[k,k] = 1   #Marks which f(r_i) have contributed to the construction of the square
...
def qs(n, smoothness_bound = 0, LOG_TOLERANCE = 20, LOG_FUNCTION = log_function, OVERKILL = 1,
 SIEVE_INTERVAL_SIZE = 0, CHECK_EVERY_SQUARE_COMBINATION = False, PRINT_MATRICES = False, PRINT_INFO = True, SURPRESS_WARNING = False):
    """
    ...
    if len(smooth_f_r_i) > 0:
        ...
        """ count = 0
        while not found_factor and count < len(smooth_f_r_i):
            print("Factor not found in previous round of Gaussian elimination. Attempting permutations.")
            smooth_f_r_i = smooth_f_r_i[1:] + [smooth_f_r_i[0]]   #Shifts the list of smooth f(r_i) by 1
            found_factor = gaussian_elimination_step(n, smooth_f_r_i, factor_base, PRINT_MATRICES, PRINT_INFO, CHECK_EVERY_SQUARE_COMBINATION)
            count += 1 """
...
bar_plot = best_results[['n','R(1)','R(a)','R(b)']].plot.barh(x = 'n',figsize=(5,9))
...
bar_plot.get_figure().savefig('D:\Documents\#University\Year 3\Project\Report\multiplier_bar_chart_poster', bbox_inches = 'tight')

# %%
qs(2**67-1, OVERKILL=20, LOG_TOLERANCE=20, PRINT_INFO=False)
